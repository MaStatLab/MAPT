...
markov.apt.2D.fit = markov.apt.density.kD(obs.mat,max.dim=max.dim,rho0=0.1,rho0.mode=0,n.s=5,tran.mode=2,beta=0,lognu.lb=-1,lognu.ub=4,n.grid=5,x.mat.pred=xy.grid,n.post.samples = n.post.sample)

for (i in 1:n.post.sample) {
part.post.sample = markov.apt.2D.fit$part.post.samples[[i]]
